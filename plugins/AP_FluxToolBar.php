...
$images = array();
$d = dir(PUN_ROOT.'img/fluxtoolbar/'.$ftb_conf['img_pack']);
...
function validate_tag($t, $mode, $name = '')
{
	...
	if ($t['image'] != $old['image'] && !file_exists(PUN_ROOT.'img/fluxtoolbar/'.$ftb_conf['img_pack'].'/'.$t['image']))
...
else if (isset($_POST['delete_img']))
{
	...
	else
	{
		...
		{
			if (!@unlink(PUN_ROOT.'img/fluxtoolbar/'.$ftb_conf['img_pack'].'/'.$img))
				$not_deleted[] = $img;
		}
	}
...
else if (isset($_POST['add_image']))
{
	...
	if (is_uploaded_file($uploaded_file['tmp_name']))
	{
		...
		if (!@move_uploaded_file($uploaded_file['tmp_name'], PUN_ROOT.'img/fluxtoolbar/'.$ftb_conf['img_pack'].'/'.$filename.'.tmp'))
			message($lang_ftb_admin['move_failed']);

		// Now check the width/height
		list($width, $height, $type,) = getimagesize(PUN_ROOT.'img/fluxtoolbar/'.$ftb_conf['img_pack'].'/'.$filename.'.tmp');
		if (empty($width) || empty($height) || $width > $ftb_conf['button_width'] || $height > $ftb_conf['button_height'])
		{
			@unlink(PUN_ROOT.'img/fluxtoolbar/'.$ftb_conf['img_pack'].'/'.$filename.'.tmp');
		...
		@unlink(PUN_ROOT.'img/fluxtoolbar/'.$ftb_conf['img_pack'].'/'.$filename.$extensions[0]);
		@unlink(PUN_ROOT.'img/fluxtoolbar/'.$ftb_conf['img_pack'].'/'.$filename.$extensions[1]);
		@unlink(PUN_ROOT.'img/fluxtoolbar/'.$ftb_conf['img_pack'].'/'.$filename.$extensions[2]);
		@rename(PUN_ROOT.'img/fluxtoolbar/'.$ftb_conf['img_pack'].'/'.$filename.'.tmp', PUN_ROOT.'img/fluxtoolbar/'.$ftb_conf['img_pack'].'/'.$filename.$extensions[0]);
		@chmod(PUN_ROOT.'img/fluxtoolbar/'.$ftb_conf['img_pack'].'/'.$filename.$extensions[0], 0644);
	}
	else
...
<?php
	$packs = array();
	$d = dir(PUN_ROOT.'img/fluxtoolbar');
	while (($entry = $d->read()) !== false)
	{
		 if ($entry != '.' && $entry != '..' && is_dir(PUN_ROOT.'img/fluxtoolbar/'.$entry))
			$packs[] = $entry;
	}
	$d->close();
	@natsort($packs);
